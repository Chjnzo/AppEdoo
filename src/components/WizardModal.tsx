...
const WizardModal = ({ open, onOpenChange }: WizardModalProps) => {
  ...
  const handleClose = (isOpen: boolean) => {
    onOpenChange(isOpen);
    if (!isOpen) {
      setTimeout(resetWizard, 300); // Reset after animation
    }
  };
        ...
        <div className="p-8">
          <DialogHeader className="mb-8">
            ...
            <DialogDescription className="text-zinc-500">
              {currentWizardStep === 1 && "Enter the basic information for this educational week."}
              {currentWizardStep === 2 && "Upload photos and details for each student."}
              {currentWizardStep === 3 && "Organize your students into working groups."}
            </DialogDescription>
          </DialogHeader>
